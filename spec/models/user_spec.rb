...
describe User do
  ...
  describe "full name from first name and last name" do
    its(:full_name) { should == "#{@user.first_name} #{@user.last_name}" }
  end

  describe "handle from email" do
    its(:handle) { should == "@@#{@user.email}" }
  end
...
#        user_with_same_nick.email = "otheremail@glowfori.com"
...
#      before do
#        user_with_same_nick = @user.dup
#        user_with_same_nick.email = "otheremail@glowofori.com"
...
#Micropost section
  describe "micropost associations" do
    ...
    let!(:older_micropost) do
      FactoryGirl.create(:micropost, user: @user, to: @user, created_at: 1.day.ago)
    end
    let!(:newer_micropost) do
      FactoryGirl.create(:micropost, user: @user, to: @user, created_at: 1.hour.ago)
    end
  ...
  describe "handling replies" do
    ...
    it "should scope replies to self" do
      @user.save
      m = @user.microposts.create(content:"@@test@example.com to donald")
